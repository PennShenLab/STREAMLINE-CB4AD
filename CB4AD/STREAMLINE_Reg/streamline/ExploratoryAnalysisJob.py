...
def job_2(dataset_path,experiment_path,cv_partitions,partition_method,categorical_cutoff,export_feature_correlations,export_univariate_plots,class_label,instance_label,match_label,random_state,ignore_features_path,categorical_feature_path,sig_cutoff,jupyterRun):
    """ Prepares ignore_features and categorical_feature_headers lists then calls the exploratory analyisis method."""
    #Allows user to specify features in target datasets that should be excluded during pipeline analysis
    if ignore_features_path == 'None':
        ignore_features = []
    ...
    if categorical_feature_path == 'None':
        ...
        categorical_feature_headers = pd.read_csv(categorical_feature_path,sep=',')
        categorical_feature_headers = list(categorical_feature_headers)
    runExplore_2(dataset_path,experiment_path,cv_partitions,partition_method,categorical_cutoff,export_feature_correlations,export_univariate_plots,class_label,instance_label,match_label,random_state,ignore_features,categorical_feature_headers,sig_cutoff,jupyterRun)
...
# def runExplore_2(dataset_path,experiment_path,cv_partitions,partition_method,categorical_cutoff,export_feature_correlations,export_univariate_plots,class_label,instance_label,match_label,random_state,ignore_features,categorical_feature_headers,sig_cutoff,jupyterRun):
#     """ Run all elements of the exploratory analysis: basic data cleaning, automated identification of categorical vs. quantitative features, basic data summary (e.g. sample size, feature type counts, class counts)"""
#     job_start_time = time.time() #for tracking phase runtime
#     topFeatures = 20 # only used with jupyter notebook reporting of univariate analyses.
#     #Set random seeds for replicatability
...
#     np.random.seed(random_state)
#     #Make analysis folder for target dataset and a folder for the respective exploratory analysis within it
#     dataset_name,dataset_ext = makeFolders_2(dataset_path,experiment_path)
#     #Load target dataset
#     if eval(jupyterRun):
#         print("Loading Dataset: "+str(dataset_name))
#     data = loadData(dataset_path,dataset_ext)
...
#     if eval(jupyterRun):
#         print("Identifying Feature Types...")
#     categorical_variables = idFeatureTypes_2(x_data,categorical_feature_headers,categorical_cutoff,experiment_path,dataset_name)
#     #Export basic exploratory analysis files
#     if eval(jupyterRun):
#         print("Running Basic Exploratory Analysis...")
#     describeData_2(data,experiment_path,dataset_name)
#     totalMissing = missingnessCounts_2(data,experiment_path,dataset_name,jupyterRun)
#     countsSummary_2(data,class_label,experiment_path,dataset_name,instance_label,match_label,categorical_variables,totalMissing,jupyterRun)

#     #Export feature correlation plot if user specified
#     if eval(export_feature_correlations):
#         if eval(jupyterRun):
#             print("Generating Feature Correlation Heatmap...")
#         featureCorrelationPlot_2(x_data,experiment_path,dataset_name,jupyterRun)
#     #Export feature labels from data header as a reference to be used later in the pipeline
#     reportHeaders_2(x_data,experiment_path,dataset_name)
#     del x_data #memory cleanup
#     #Conduct univariate analyses of association between individual features and class
#     if eval(jupyterRun):
#         print("Running Univariate Analyses...")
...
#         print("Generating and Saving CV Datasets...")
#     train_dfs,test_dfs = cv_partitioner(data,cv_partitions,partition_method,class_label,match_label,random_state)
# #    train_dfs,test_dfs = cv_partitioner_2(data,cv_partitions,partition_method,class_label,match_label,random_state)
#     saveCVDatasets_2(experiment_path,dataset_name,train_dfs,test_dfs)
#     #Save phase runtime
#     saveRuntime_2(experiment_path,job_start_time)
#     #Print phase completion
#     print(dataset_name+" phase 1 complete")
#     job_file = open(experiment_path + '/jobsCompleted/job_exploratory_'+dataset_name+'.txt', 'w')
#     job_file.write('complete')
...
# def makeFolders_2(dataset_path,experiment_path):
#     """ Make analysis folder for target dataset and a folder for the respective exploratory analysis within it"""
#     dataset_name = dataset_path.split('/')[-1].split('.')[0]
#     dataset_ext = dataset_path.split('/')[-1].split('.')[-1]
#     if not os.path.exists(experiment_path + '/' + 'L21'):
#         os.mkdir(experiment_path + '/' + 'L21')
#     if not os.path.exists(experiment_path + '/' + 'L21' + '/exploratory'):
#         os.mkdir(experiment_path + '/' + 'L21' + '/exploratory')
#     return dataset_name,dataset_ext
...
# def idFeatureTypes_2(x_data,categorical_feature_headers,categorical_cutoff,experiment_path,dataset_name):
#     """ Takes a dataframe (of independent variables) with column labels and returns a list of column names identified as
#     being categorical based on user defined cutoff (categorical_cutoff). """
#     #Identify categorical variables in dataset
#     if len(categorical_feature_headers) == 0: #Runs unless user has specified a predifined list of variables to treat as categorical
...
#         categorical_variables = categorical_feature_headers
#     #Pickle list of feature names to be treated as categorical variables
#     outfile = open(experiment_path + '/' + 'L21' + '/exploratory/categorical_variables.pickle', 'wb')
#     pickle.dump(categorical_variables, outfile)
#     outfile.close()
#     return categorical_variables
...
# def describeData_2(data,experiment_path,dataset_name):
#     """Conduct and export basic dataset descriptions including basic column statistics, column variable types (i.e. int64 vs. float64),
#         and unique value counts for each column"""
#     data.describe().to_csv(experiment_path + '/' + 'L21' + '/exploratory/'+'DescribeDataset.csv')
#     data.dtypes.to_csv(experiment_path + '/' + 'L21' + '/exploratory/'+'DtypesDataset.csv',header=['DataType'],index_label='Variable')
#     data.nunique().to_csv(experiment_path + '/' + 'L21' + '/exploratory/'+'NumUniqueDataset.csv',header=['Count'],index_label='Variable')
...
# def missingnessCounts_2(data,experiment_path,dataset_name,jupyterRun):
#     """ Count and export missing values for all data columns. Also plots a histogram of missingness across all data columns."""
#     #Assess Missingness in all data columns
#     missing_count = data.isnull().sum()
#     totalMissing = data.isnull().sum().sum()
#     missing_count.to_csv(experiment_path + '/' + 'L21' + '/exploratory/'+'DataMissingness.csv',header=['Count'],index_label='Variable')
#     #Plot a histogram of the missingness observed over all columns in the dataset
#     #plt.hist(missing_count,bins=data.shape[0]) #To view the full spectrum of posssible missingness
#     plt.hist(missing_count,bins=100)
#     plt.xlabel("Missing Value Counts")
#     plt.ylabel("Frequency")
#     plt.title("Histogram of Missing Value Counts in Dataset")
#     plt.savefig(experiment_path + '/' + 'L21' + '/exploratory/'+'DataMissingnessHistogram.png',bbox_inches='tight')
#     if eval(jupyterRun):
#         plt.show()
#     else:
#         plt.close('all')
...
# def countsSummary_2(data,class_label,experiment_path,dataset_name,instance_label,match_label,categorical_variables,totalMissing,jupyterRun):
#     """ Reports various dataset counts: i.e. number of instances, total features, categorical features, quantitative features, and class counts.
#         Also saves a simple bar graph of class counts."""
#     #Calculate, print, and export instance and feature counts
#     fCount = data.shape[1]-1
...
#     summary = [['instances',data.shape[0]],['features',fCount],['categorical_features',len(categorical_variables)],['quantitative_features',fCount - len(categorical_variables)],['missing_values',totalMissing],['missing_percent',round(percentMissing,5)]]
#     dfSummary = pd.DataFrame(summary, columns = ['Variable','Count'])
#     dfSummary.to_csv(experiment_path + '/' + 'L21' + '/exploratory/'+'DataCounts.csv',index=None)
#     #Calculate, print, and export class counts
#     class_counts = data[class_label].value_counts()
#     class_counts.to_csv(experiment_path + '/' + 'L21' + '/exploratory/'+'ClassCounts.csv',header=['Count'],index_label='Class')
#     print('Class Counts: ----------------')
#     print(class_counts)
#     #Generate and export class count bar graph
#     class_counts.plot(kind='bar')
#     plt.ylabel('Count')
#     plt.title('Class Counts')
#     plt.savefig(experiment_path + '/' + 'L21' + '/exploratory/'+'ClassCountsBarPlot.png',bbox_inches='tight')
#     if eval(jupyterRun):
#         plt.show()
#     else:
#         plt.close('all')
...
# def featureCorrelationPlot_2(x_data,experiment_path,dataset_name,jupyterRun):
#     """ Calculates feature correlations via pearson correlation and explorts a respective heatmap visualization. Due to computational expense
#         this may not be recommended for datasets with a large number of instances and/or features unless needed. The generated heatmap will be
#         difficult to read with a large number of features in the target dataset."""
#     #Calculate correlation matrix
...
#     f,ax=plt.subplots(figsize=(40,20))
#     sns.heatmap(corrmat,vmax=1,square=True)
#     plt.savefig(experiment_path + '/' + 'L21' + '/exploratory/'+'FeatureCorrelations.png',bbox_inches='tight')
#     if eval(jupyterRun):
#         plt.show()
#     else:
#         plt.close('all')
...
# def reportHeaders_2(x_data,experiment_path,dataset_name):
#     """ Exports dataset header labels for use as a reference later in the pipeline. """
#     #Get and Export Original Headers
#     headers = x_data.columns.values.tolist()
#     with open(experiment_path + '/' + 'L21' + '/exploratory/OriginalFeatureNames.csv',mode='w', newline="") as file:
#         writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
#         writer.writerow(headers)
#     file.close()
...
# def saveCVDatasets_2(experiment_path,dataset_name,train_dfs,test_dfs):
#     """ Saves individual training and testing CV datasets as .csv files"""
#     #Generate folder to contain generated CV datasets
#     if not os.path.exists(experiment_path + '/' + 'L21' + '/CVDatasets'):
#         os.mkdir(experiment_path + '/' + 'L21' + '/CVDatasets')
#     #Export training datasets
#     counter = 0
#     for each in train_dfs:
#         a = each.values
#         with open(experiment_path + '/' + 'L21' + '/CVDatasets/'+dataset_name+'_CV_' + str(counter) +"_Train.csv", mode="w", newline="") as file:
#             writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
#             writer.writerow(each.columns.values.tolist())
#             for row in a:
#                 writer.writerow(row)
...
#     for each in test_dfs:
#         a = each.values
#         with open(experiment_path + '/' + 'L21' + '/CVDatasets/'+dataset_name+'_CV_' + str(counter) +"_Test.csv", mode="w", newline="") as file:
#             writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
#             writer.writerow(each.columns.values.tolist())
#             for row in a:
#                 writer.writerow(row)
...
# def saveRuntime_2(experiment_path,job_start_time):
#     """ Export runtime for this phase of the pipeline on current target dataset"""
#     if not os.path.exists(experiment_path + '/' + 'L21' + '/runtime'):
#         os.mkdir(experiment_path + '/' + 'L21' + '/runtime')
#     runtime_file = open(experiment_path + '/' + 'L21' + '/runtime/runtime_exploratory.txt','w')
#     runtime_file.write(str(time.time()-job_start_time))
#     runtime_file.close()
