...
def submitLocalJob(experiment_path,training,rep_data_path,dataset_for_rep):
    """ Runs PDF_ReportJob_Reg.py locally, once. """
    PDF_ReportJob_Reg.job(experiment_path,training,rep_data_path,dataset_for_rep)

def submitClusterJob(experiment_path,training,rep_data_path,dataset_for_rep,reserved_memory,maximum_memory,queue):
    """ Runs PDF_ReportJob_Reg.py once. Runs on a linux-based computing cluster that uses an IBM Spectrum LSF for job scheduling."""
    ...
    sh_file.write('python ' + this_file_path + '/PDF_ReportJob_Reg.py ' + experiment_path+' '+training+' '+rep_data_path+' '+dataset_for_rep + '\n')
    sh_file.close()
