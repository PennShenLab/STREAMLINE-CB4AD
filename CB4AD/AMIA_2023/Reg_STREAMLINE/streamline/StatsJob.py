...
def job(full_path,plot_FI_box,class_label,instance_label,cv_partitions,scale_data,plot_metric_boxplots,primary_metric,top_model_features,sig_cutoff,metric_weight,jupyterRun):
...
def residualReg(algorithms,original_headers,cv_partitions,full_path,data_name,instance_label,class_label,abbrev,colors,jupyterRun):
    ...
    for i in range(len(algorithms)):
        df = pd.DataFrame([s_res_trains[i], [algorithms[i]]*len(s_res_trains[i]), ["Training"]*len(s_res_trains[i])]).transpose()
        df.columns = ["Residual", "Algorithm", "Type"]
        train_df.append(df)
    train_df = pd.concat(train_df).reset_index(drop=True)
    for i in range(len(algorithms)):
        df = pd.DataFrame([s_res_tests[i], [algorithms[i]]*len(s_res_tests[i]), ["Testing"]*len(s_res_tests[i])]).transpose()
        df.columns = ["Residual", "Algorithm", "Type"]
        test_df.append(df)
    test_df = pd.concat(test_df).reset_index(drop=True)
    
    train_df.to_csv(full_path + '/model_evaluation/residual_train.csv')
    train_df = pd.read_csv(full_path + '/model_evaluation/residual_train.csv')
