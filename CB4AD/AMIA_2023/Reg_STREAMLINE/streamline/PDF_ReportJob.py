...
def job(experiment_path,training,rep_data_path,data_path, do_L21Series):
    ...
    if eval(training):
        ...
        ds = sorted(ds)
    if eval(training):
        ds_2 = os.listdir(experiment_path)
        nonds_2 = ['DatasetComparisons', 'jobs', 'jobsCompleted', 'logs','KeyFileCopy','metadata.pickle','metadata.csv','algInfo.pickle',experiment_name+'_ML_Pipeline_Report.pdf']
        for i in nonds_2:
            if i in ds_2:
                ds_2.remove(i)
        if '.idea' in ds_2:
    ...
    ls2 = ars_dic[87:144]   # ML modeling algorithms (NaiveB - ExSTraCS) [87:132]
    ls3 = ars_dic[144:162]  # primary metric - ExportHyperparemterSweepPLot  [132:150]
    ls4 = ars_dic[162:177]  # DoLCSHyperparemeterSweep LCS hypersweep timeout) [150:165]
    ls5 = ars_dic[177:]  # ExportROCPlot to Top Model Features to Display [165:180]
...
#     #NEXT PAGE - L21 Series Weight Heat Map
# ---------------------------------------
#     if eval(training):
#         print("Publishing Weight Heat Map Summary")
#         analy_report.add_page()
#         analy_report.set_font('Times', 'B', 12)
#         analy_report.cell(w=0, h = 8, txt="Weight Heat Map Summary", border=1, align="L", ln=2)
#         analy_report.image(experiment_path+'/'+'L21'+'/models/feature_importance/Compare_FI_Norm_Weight.png', 1, 159, 208, 125)
    ...
    if eval(training):
        ...
        pageCount = math.ceil(pageCount) #rounds up to next full integer
        for page in range(0, pageCount): #generate each page
            pubRuntime(analy_report,experiment_path,ds_2,page,resultLimit,pageCount)
...
def pubModelStats(analy_report,experiment_path,ds,page,resultLimit,pageCount,training,train_name):
    ...
    analy_report.y += 4
    analy_report.set_font('Times', 'B', 10)
    analy_report.multi_cell(w=0, h=4, txt='L21 Series', border=1, align='L')
    analy_report.y += 1 #Space below section header
    analy_report.set_font('Times','', 7)
        
    stats_ds_2 = pd.read_csv(experiment_path+'/'+'L21'+"/model_evaluation/Summary_performance_mean.csv",sep=',',index_col=0)
    stats_ds_2.reset_index(inplace=True)
    stats_ds_2 = stats_ds_2.columns.to_frame().T.append(stats_ds_2, ignore_index=True)
    stats_ds_2.columns = range(len(stats_ds_2.columns))
    epw = 208 #Amount of Space (width) Avaliable
    th = analy_report.font_size
    col_width_list = [32, 21, 21, 21, 21, 21, 21, 21, 21]
    table1 = stats_ds_2.iloc[: , :]
    table1 = table1.to_numpy()
