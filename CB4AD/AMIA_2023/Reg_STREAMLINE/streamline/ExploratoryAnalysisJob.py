...
def countsSummary(data,class_label,experiment_path,dataset_name,instance_label,match_label,categorical_variables,totalMissing,jupyterRun):
    ...
    print('Label Counts: ----------------')
    print(class_counts)
    #Generate and export label count histogram graph
    plt.figure()
    plt.hist(data[class_label], bins = 100)
    plt.ylabel('Count')
    plt.xlabel('Label')
    plt.title('Label Counts')
    plt.savefig(experiment_path + '/' + dataset_name + '/exploratory/'+'ClassCountsBarPlot.png',bbox_inches='tight')
    print("The Skewness value of the labels is: ", scs.skew(data[class_label]))
    print("The Kurtosis value of the labels is: ", scs.kurtosis(data[class_label]))
    if eval(jupyterRun):
        plt.show()
    else:
        plt.close('all')
