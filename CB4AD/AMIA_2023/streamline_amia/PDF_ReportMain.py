...
def main(argv):
    ...
    parser.add_argument('--class-number', dest='class_number', type=int, help='# of input data classes',default=2)
    parser.add_argument('--alg-n', dest='alg_n', type=int, help='# of algorithms selected',default=13)
    ...
    if not options.do_check: #Run job submission
        ...
        if eval(options.run_parallel):
            if (options.class_number==2):
                ...
                submitClusterJobMulti(experiment_path,options.alg_n,options.training,options.rep_data_path,options.dataset_for_rep,options.reserved_memory,options.maximum_memory,options.queue)
        else:
            if (options.class_number==2):
                submitLocalJob(experiment_path,options.training,options.rep_data_path,options.dataset_for_rep)
            else:
                submitLocalJobMulti(experiment_path,options.alg_n,options.training,options.rep_data_path,options.dataset_for_rep)
...
def submitLocalJobMulti(experiment_path,alg_n,training,rep_data_path,dataset_for_rep):
    """ Runs PDF_ReportJob.py locally, once. """
    PDF_ReportJobMulti.job(experiment_path,alg_n,training,rep_data_path,dataset_for_rep)
...
def submitClusterJobMulti(experiment_path,alg_n,training,rep_data_path,dataset_for_rep,reserved_memory,maximum_memory,queue):
    """ Runs PDF_ReportJob.py once. Runs on a linux-based computing cluster that uses an IBM Spectrum LSF for job scheduling."""
    job_ref = str(time.time())
    job_name = experiment_path + '/jobs/PDF_' + job_ref + 'Multi' + '_run.sh'
    sh_file = open(job_name,'w')
    sh_file.write('#!/bin/bash\n')
    sh_file.write('#BSUB -q '+queue+'\n')
    sh_file.write('#BSUB -J '+job_ref+'\n')
    sh_file.write('#BSUB -R "rusage[mem='+str(reserved_memory)+'G]"'+'\n')
    sh_file.write('#BSUB -M '+str(maximum_memory)+'GB'+'\n')
    sh_file.write('#BSUB -o ' + experiment_path+'/logs/PDF_'+job_ref+'Multi'+'.o\n')
    sh_file.write('#BSUB -e ' + experiment_path+'/logs/PDF_'+job_ref+'Multi'+'.e\n')

    this_file_path = os.path.dirname(os.path.realpath(__file__))
    sh_file.write('python ' + this_file_path + '/PDF_ReportJobMulti.py ' + experiment_path+' '+alg_n+' '+training+' '+rep_data_path+' '+dataset_for_rep + '\n')
    sh_file.close()
    os.system('bsub < ' + job_name)
    pass
