...
def job(experiment_path,alg_n,training,rep_data_path,data_path):
    ...
    for n in range(len(ds)):
        ...
        analy_report.x = 1
        analy_report.y = 1
        analy_report.cell(32, 4, 'Confusion Matrix', 1, align="L")
        if eval(training):
            for img_n in range(0,min(alg_n,3)):
                analy_report.image(experiment_path+'/'+ds[n]+'/model_evaluation/CM_'+str(img_n+1)+'.png', 4+68*img_n, 5, 68)
            for img_n in range(0,min(alg_n-3,3)):
                analy_report.image(experiment_path+'/'+ds[n]+'/model_evaluation/CM_'+str(img_n+4)+'.png', 4+68*img_n, 75, 68)
            for img_n in range(0,min(alg_n-6,3)):
                analy_report.image(experiment_path+'/'+ds[n]+'/model_evaluation/CM_'+str(img_n+7)+'.png', 4+68*img_n, 145, 68)
            for img_n in range(0,min(alg_n-9,3)):
                analy_report.image(experiment_path+'/'+ds[n]+'/model_evaluation/CM_'+str(img_n+10)+'.png', 4+68*img_n, 215, 68)
            for img_n in range(0,min(alg_n-12,3)):
                analy_report.image(experiment_path+'/'+ds[n]+'/model_evaluation/CM_'+str(img_n+13)+'.png', 4+68*img_n, 285, 68)
            #analy_report.image(experiment_path+'/'+ds[n]+'/model_evaluation/metricBoxplots/Compare_Balanced Accuracy.png', 74,156,82,85)
        else:
            for img_n in range(0,min(alg_n,3)):
                analy_report.image(experiment_path+'/'+train_name+'/applymodel/'+ds[n]+'/model_evaluation/CM_'+str(img_n+1)+'.png', 4+68*img_n, 5, 68)
            for img_n in range(0,min(alg_n-3,3)):
                analy_report.image(experiment_path+'/'+train_name+'/applymodel/'+ds[n]+'/model_evaluation/CM_'+str(img_n+4)+'.png', 4+68*img_n, 75, 68)
            for img_n in range(0,min(alg_n-6,3)):
                analy_report.image(experiment_path+'/'+train_name+'/applymodel/'+ds[n]+'/model_evaluation/CM_'+str(img_n+7)+'.png', 4+68*img_n, 145, 68)
            for img_n in range(0,min(alg_n-9,3)):
                analy_report.image(experiment_path+'/'+train_name+'/applymodel/'+ds[n]+'/model_evaluation/CM_'+str(img_n+10)+'.png', 4+68*img_n, 215, 68)
            for img_n in range(0,min(alg_n-12,3)):
                analy_report.image(experiment_path+'/'+train_name+'/applymodel/'+ds[n]+'/model_evaluation/CM_'+str(img_n+13)+'.png', 4+68*img_n, 285, 68)
            #analy_report.image(experiment_path+'/'+train_name+'/applymodel/'+ds[n]+'/model_evaluation/metricBoxplots/Compare_Balanced Accuracy.png', 74,156,82,85)
        footer(analy_report)
    ...
    if eval(training):
        ...
        for k in range(len(ds)):
            ...
            analy_report.image(experiment_path+'/'+ds[k]+'/model_evaluation/feature_importance/Permutation/Compare_FI_Norm_Weight.png', 1, 159, 208, 125) #130 added
            footer(analy_report)
    ...
    if eval(training):
        ...
        analy_report.set_font(family='times', size=9)
        if len(ds) > 1:
            analy_report.image(experiment_path+'/DatasetComparisons/dataCompBoxplots/'+'DataCompareAllModels_Accuracy.png',  1, 12, 208, 130) #Images adjusted to fit a width of 100 and length of 135
            analy_report.image(experiment_path+'/DatasetComparisons/dataCompBoxplots/'+'DataCompareAllModels_Balanced Accuracy.png',  1, 150, 208, 130) #Images adjusted to fit a width of 100 and length of 135
        footer(analy_report)
