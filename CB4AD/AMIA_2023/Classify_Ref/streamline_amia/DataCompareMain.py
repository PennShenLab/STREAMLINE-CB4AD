...
def main(argv):
    ...
    if not options.do_check: #Run job submission
        ...
        if eval(options.run_parallel):
            submitClusterJob(options.output_path+'/'+options.experiment_name,options.reserved_memory,options.maximum_memory,options.queue,sig_cutoff,options.class_number,jupyterRun)
        else:
            submitLocalJob(options.output_path+'/'+options.experiment_name,sig_cutoff,options.class_number,jupyterRun)
...
def submitLocalJob(experiment_path,sig_cutoff,class_number,jupyterRun):
    """ Runs DataCompareJob.py locally, once. """
    DataCompareJob.job(experiment_path,sig_cutoff,class_number,jupyterRun)

def submitClusterJob(experiment_path,reserved_memory,maximum_memory,queue,sig_cutoff,cluss_number,jupyterRun):
    ...
    sh_file.write('python ' + this_file_path + '/DataCompareJob.py ' + experiment_path +" "+ str(sig_cutoff)+" "+str(class_number)+" "+ str(jupyterRun)+ '\n')
    sh_file.close()
