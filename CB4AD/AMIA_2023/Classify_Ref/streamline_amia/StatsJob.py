...
def job(full_path,plot_ROC,plot_PRC,plot_FI_box,class_label,instance_label,cv_partitions,scale_data,plot_metric_boxplots,primary_metric,top_model_features,sig_cutoff,metric_weight,jupyterRun):
    ...
    fi_df_list,fi_ave_list,fi_ave_norm_list,ave_metric_list,all_feature_list,non_zero_union_features,non_zero_union_indexes,fi_ave_frac_list = prepFI(algorithms,full_path,abbrev,metric_dict,metric_weight) #old - 'Balanced Accuracy'
    #Select 'top' features for composite vizualization

    class_number=2 # binary classificaion
    featuresToViz_n,featuresToViz_f,featuresToViz_nw,featuresToViz_fw,scoreSumDict_features_n,scoreSumDict_features_f,scoreSumDict_features_nw,scoreSumDict_features_fw = selectForCompositeViz(top_model_features,non_zero_union_features,non_zero_union_indexes,algorithms,ave_metric_list,fi_ave_norm_list,fi_ave_frac_list,class_number)
    
    saveFISummary(full_path,scoreSumDict_features_n,scoreSumDict_features_f,scoreSumDict_features_nw,scoreSumDict_features_fw,"P")
    ...
    if eval(plot_FI_box):
        ...
        doFIBoxplots(full_path,fi_df_list,fi_ave_list,algorithms,original_headers,top_model_features,"P",jupyterRun)
        doFI_Histogram(full_path, fi_ave_list, algorithms, "P",jupyterRun)
    ...
    top_fi_ave_norm_list,all_feature_listToViz_norm = getFI_To_Viz_Sorted(featuresToViz_n,all_feature_list,algorithms,fi_ave_norm_list) #If we want composite FI plots to be displayed in descenting total bar height order.
    top_fi_ave_frac_list,all_feature_listToViz_frac = getFI_To_Viz_Sorted(featuresToViz_f,all_feature_list,algorithms,fi_ave_frac_list)
    
    # Permutation FI
    #Generate Normalized composite FI plot
    composite_FI_plot(top_fi_ave_norm_list, algorithms, list(colors.values()), all_feature_listToViz_norm, 'Norm',full_path,jupyterRun, 'Normalized Feature Importance',"P")
    #Generate Fractioned composite FI plot
    composite_FI_plot(top_fi_ave_frac_list, algorithms, list(colors.values()), all_feature_listToViz_frac, 'Frac',full_path,jupyterRun, 'Fractioned Feature Importance',"P")
    ...
    composite_FI_plot(fracLists, algorithms, list(colors.values()), all_feature_listToViz_norm, 'Norm_Frac',full_path,jupyterRun, 'Normalized and Fractioned Feature Importance',"P")
    
    #Weight FI scores for normalized and (model performance) weighted composite FI plot
    weightedLists_norm,weights = weightFI(ave_metric_list,fi_ave_norm_list,class_number)
    weightedLists_frac,weights = weightFI(ave_metric_list,fi_ave_frac_list,class_number)
    top_fi_ave_normw_list,all_feature_listToViz_normw = getFI_To_Viz_Sorted(featuresToViz_nw,all_feature_list,algorithms,weightedLists_norm) #If we want composite FI plots to be displayed in descenting total bar height order.
    top_fi_ave_fracw_list,all_feature_listToViz_fracw = getFI_To_Viz_Sorted(featuresToViz_fw,all_feature_list,algorithms,weightedLists_frac)
    #Generate Normalized and Weighted Compount FI plot
    composite_FI_plot(top_fi_ave_normw_list, algorithms, list(colors.values()), all_feature_listToViz_normw, 'Norm_Weight',full_path,jupyterRun, 'Normalized and Weighted Feature Importance',"P")
    composite_FI_plot(top_fi_ave_fracw_list, algorithms, list(colors.values()), all_feature_listToViz_fracw, 'Frac_Weight',full_path,jupyterRun, 'Fractioned and Weighted Feature Importance',"P")
    ...
    composite_FI_plot(weightedFracLists, algorithms, list(colors.values()), all_feature_listToViz_normw, 'Norm_Frac_Weight',full_path,jupyterRun, 'Normalized, Fractioned, and Weighted Feature Importance',"P")
...
############################################
def save_FI(FI_all,algorithm,globalFeatureList,full_path,FI_type):
    ...
    if not os.path.exists(full_path+'/model_evaluation/feature_importance/'):
        ...
        os.mkdir(full_path+'/model_evaluation/feature_importance/Permutation/')
        os.mkdir(full_path+'/model_evaluation/feature_importance/Permutation/FI_summary/')
    filepath = full_path+'/model_evaluation/feature_importance/'+FI_type+'/'+algorithm+"_FI.csv"
    ...
    with open(full_path+'/model_evaluation/Summary_performance_mean.csv',mode='w', newline="") as file:
        ...
        for algorithm in metric_dict:
            ...
            for l in list(metric_dict[algorithm].values()):
                ...
                meani = np.mean(l) #mean(l)
                std = np.std(l) #stdev(l)
...
def prepFI(algorithms,full_path,abbrev,metric_dict,metric_weight):
    ...
    for algorithm in algorithms:
        ...
        temp_df = pd.read_csv(full_path+'/model_evaluation/feature_importance/Permutation/'+abbrev[algorithm]+"_FI.csv") #CV FI scores for all original features in dataset.
        ...
        avgBA = np.mean(metric_dict[algorithm][metric_weight]) #   old-     avgBA = mean(metric_dict[algorithm][primary_metric])
    ...
    #Fraction Average Feature importance scores so they fall between (0 - 1)
    fi_ave_frac_list = []
    for each in fi_ave_list:  # each algorithm
        fracList = []
        sumList = each
        ...
        for i in range(len(each)):
            if sum(sumList)==0:
                ...
                fracList.append((each[i]) / (sum(sumList)))
        fi_ave_frac_list.append(fracList)
    ...
    return fi_df_list,fi_ave_list,fi_ave_norm_list,ave_metric_list,all_feature_list,non_zero_union_features,non_zero_union_indexes,fi_ave_frac_list

def selectForCompositeViz(top_model_features,non_zero_union_features,non_zero_union_indexes,algorithms,ave_metric_list,fi_ave_norm_list,fi_ave_frac_list,class_number):
    ...
    featuresToViz_n = None
    featuresToViz_f = None
    featuresToViz_nw = None
    featuresToViz_fw = None
    ...
    for each in non_zero_union_features:  # for each non-zero feature
        for j in range(len(algorithms)):  # for each algorithm
            ...
            score_n = fi_ave_norm_list[j][non_zero_union_indexes[i]]
            score_f = fi_ave_frac_list[j][non_zero_union_indexes[i]]
            # multiply score by algorithm performance weight
            weight = ave_metric_list[j]
            thre = 1/class_number
            if weight <= thre: #This is why this method is limited to balanced_accuracy and roc_auc
                weight = 0
            if not weight == 0:
                weight = (weight - thre) / thre
            score_nw = score_n * weight
            score_fw = score_f * weight
            #score = score * ave_metric_list[j]
            if not each in scoreSumDict_n:
                scoreSumDict_n[each] = score_n
            else:
                scoreSumDict_n[each] += score_n
            if not each in scoreSumDict_f:
                scoreSumDict_f[each] = score_f
            else:
                scoreSumDict_f[each] += score_f
            if not each in scoreSumDict_nw:
                scoreSumDict_nw[each] = score_nw
            else:
                scoreSumDict_nw[each] += score_nw
            if not each in scoreSumDict_fw:
                scoreSumDict_fw[each] = score_fw
            else:
                scoreSumDict_fw[each] += score_fw
        i += 1
    # Sort features by decreasing score
    scoreSumDict_features_n = sorted(scoreSumDict_n, key=lambda x: scoreSumDict_n[x], reverse=True)
    scoreSumDict_features_f = sorted(scoreSumDict_f, key=lambda x: scoreSumDict_f[x], reverse=True)
    scoreSumDict_features_nw = sorted(scoreSumDict_nw, key=lambda x: scoreSumDict_nw[x], reverse=True)
    scoreSumDict_features_fw = sorted(scoreSumDict_fw, key=lambda x: scoreSumDict_fw[x], reverse=True)
    if len(non_zero_union_features) > top_model_features: #Keep all features if there are fewer than specified top results
        featuresToViz_n = scoreSumDict_features_n[0:top_model_features]
        featuresToViz_f = scoreSumDict_features_f[0:top_model_features]
        featuresToViz_nw = scoreSumDict_features_nw[0:top_model_features]
        featuresToViz_fw = scoreSumDict_features_fw[0:top_model_features]
    else:
        featuresToViz_n = scoreSumDict_features_n
        featuresToViz_f = scoreSumDict_features_f
        featuresToViz_nw = scoreSumDict_features_nw
        featuresToViz_fw = scoreSumDict_features_fw
    return featuresToViz_n,featuresToViz_f,featuresToViz_nw,featuresToViz_fw,scoreSumDict_features_n,scoreSumDict_features_f,scoreSumDict_features_nw,scoreSumDict_features_fw #list of feature names to vizualize in composite FI plots.



def doFIBoxplots(full_path,fi_df_list,fi_ave_list,algorithms,original_headers,top_model_features, FI_type,jupyterRun):
    ...
    for algorithm in algorithms: #each algorithms
        ...
        if FI_type=="P":
            plt.savefig(full_path+'/model_evaluation/feature_importance/Permutation/' + algorithm + '_boxplot',bbox_inches="tight")
...
def doFI_Histogram(full_path, fi_ave_list, algorithms, FI_type,jupyterRun):
    ...
    for algorithm in algorithms: #each algorithms
        ...
        if FI_type=="P":
            plt.savefig(full_path+'/model_evaluation/feature_importance/Permutation/' + algorithm + '_histogram',bbox_inches="tight")
...
def composite_FI_plot(fi_list, algorithms, algColors, all_feature_listToViz, figName,full_path,jupyterRun,yLabelText,FI_type):
    ...
    #Export and/or show plot
    if FI_type=="P":
        plt.savefig(full_path+'/model_evaluation/feature_importance/Permutation/Compare_FI_' + figName + '.png', bbox_inches='tight')
...
def weightFI(ave_metric_list,top_fi_ave_nf_list,class_number):
    ...
    for i in range(len(ave_metric_list)):
        if ave_metric_list[i] <= (1/class_number):
            ave_metric_list[i] = 0
    ...
    for i in range(len(ave_metric_list)):
        if ave_metric_list[i] == 0:
            ...
            weights.append((ave_metric_list[i] - (1/class_number)) / (1/class_number))
    # Weight normalized feature importances
    weightedLists = []
    for i in range(len(top_fi_ave_nf_list)): #each algorithm
        weightList = np.multiply(weights[i], top_fi_ave_nf_list[i]).tolist()
...
def saveFISummary(full_path,scoreSumDict_features_n,scoreSumDict_features_f,scoreSumDict_features_nw,scoreSumDict_features_fw,FI_type):
    if FI_type=="P":
        with open(full_path + '/model_evaluation/feature_importance/Permutation/FI_summary/Top_FI_summary.csv', mode='w', newline="") as file:
            writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            methods=["Normalization","Fraction","Weighted Normalization","Weighted Fraction"]
            writer.writerow(methods)  
            for i in range(len(scoreSumDict_features_n)):
                writer.writerow([scoreSumDict_features_n[i],scoreSumDict_features_f[i],scoreSumDict_features_nw[i],scoreSumDict_features_fw[i]])
        file.close()
