...
from sklearn.linear_model import SGDClassifier # Added for elastic net
...
def runModel(algorithm,train_file_path,test_file_path,full_path,n_trials,timeout,lcs_timeout,export_hyper_sweep_plots,instance_label,class_label,random_state,cvCount,filter_poor_features,do_lcs_sweep,nu,iterations,N,training_subsample,use_uniform_FI,primary_metric,param_grid,algAbrev):
    ...
    if algorithm == 'Naive Bayes':
        ...
        ret = run_EN_full(trainX,trainY,testX,testY, random_state, cvCount,param_grid,n_trials,timeout,export_hyper_sweep_plots,full_path,use_uniform_FI,primary_metric)
...
def objective_EN(trial, est, x_train, y_train, random_state, hype_cv, param_grid, scoring_metric):
    ...
    params = {'penalty': trial.suggest_categorical('penalty',param_grid['penalty']),
			  ...
			  'l1_ratio' : trial.suggest_loguniform('l1_ratio',param_grid['l1_ratio'][0], param_grid['l1_ratio'][1]),
...
def objective_CGB(trial, est, x_train, y_train, random_state, hype_cv, param_grid, scoring_metric):
    ...
    params = {'learning_rate': trial.suggest_loguniform('learning_rate',param_grid['learning_rate'][0],param_grid['learning_rate'][1]),
              ...
              'random_strength' : trial.suggest_categorical('random_strength',param_grid['random_strength'])}
...
def hyperparameters(random_state,do_lcs_sweep,nu,iterations,N,feature_names): #### Add new algorithm hyperparameters here using same formatting...
    ...
    # Elastic Net
    param_grid_EN = {'penalty':['elasticnet'], 'loss':['log_loss','modified_huber'],'alpha':[1e-3,1],'max_iter': [10, 2000],                      
		    'l1_ratio':[1e-5,1-1e-5],'class_weight': [None, 'balanced'],'random_state':[random_state]}
    ...
    param_grid_CGB = {'learning_rate':[.0001, 0.3],'iterations':[10,500],'depth':[1,10],'l2_leaf_reg': [1,9],
                      'loss_function': ['Logloss'], 'random_seed': [random_state],'random_strength':[random_state]}
